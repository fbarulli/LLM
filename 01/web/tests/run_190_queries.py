...
def evaluate_config(config_name, config_path, test_queries):
    ...
    for q in tqdm(test_queries, desc="Retrieving"):
        results = manager.search_faq(q['query'], 5, q.get('expected_course'))
        if results:
            queries_data.append({
                'query': q['query'],
        ...
        for j, eval_result in enumerate(evaluations):
            all_evaluations.append({
                **batch[j],
                ...
                'success': batch[j]['expected_id'] in [hit['_id'] for hit in batch[j]['results']] if batch[j]['expected_id'] else False
            })
    ...
    print(f"\n📊 {config_name} Results ({total} queries):")
...
def main():
    print("=" * 60)
    print("EVALUATING 190 QUERIES (MERGED SET)")
    print("=" * 60)
    ...
    print("\n" + "=" * 60)
    print("📊 FINAL RESULTS (190 QUERIES)")
    print("=" * 60)
    ...
    # Save results
    with open('../experiments/190_results.json', 'w') as f:
        json.dump(results, f, indent=2)
    print("\n✅ Results saved to experiments/190_results.json")
